# Create a program to requirements
# -----------------------------------
# Instructions:
# 1. take at least one input from the user that is then displayed back to them.
# 2. use sys.stdout rather than print where possible
# 3. include at least one comment and two strings
# 4. feature a list of int data type
# 5. use an if statement
# 6. contain at least one comparison operator
# 7. include one loop, either a while-loop or a for-loop

# My Summary on the program
# -------------------------------------
# The purpose of this program is to tell the user their nett pay that they will receive into their bank account after the tax is deducted. AND what activities they can afford to do with that money.
# We need the user to input their hourly rate AND how many hours they have worked this week AND store them as a variable INT.
# while loops have been created under the hourly rate and hours worked inputs, as the program would not work well with negative numbers.
# The loops will continually ask the user to enter a positive number, if they have entered a negative number. 
# We need to store the hourly rate as a variable float to allow for mutliplication and subtraction of the values input
# We need to store the tax as a decimal variable float to allow for mutliplication and subtraction of the values input
# We need to print the amount of nett left over.
# There are three lists of activities. The nett_pay will determine which list is printed.

import sys

sys.stdout.write("Hello, We are your friendly budget consultants!")
sys.stdout.write("\n\nPlease answer the following questions. \nThe trolls downstairs will then let you know how much fun you are allowed this weekend.")
...
# note to self---------------  
# a new variable has been created called gross_pay which is the sum of 2 variables already created, hourly_pay_rate by hours_worked.
# a new variable has been created called tax, I have simply entered a sum to find 12 percent of the gross_pay. The tax declared in this program is a made up figure, however this is the value of the tax we will apply in this program.
# I've chosen to enter a decimal to find the percentage as I understand that dividing the gross pay by a percentage would not achieve the same result as multiplying the gross pay by a decimal.
# another variable is declared and named nett_pay which will be the result of subtracting the gross_pay variable from the tax variable. 
